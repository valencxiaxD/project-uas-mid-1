...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QPushButton, QGridLayout, QTableWidget, QTableWidgetItem, QComboBox, QMessageBox, QStackedWidget, QHBoxLayout, QVBoxLayout,QHeaderView, QFrame, QDialog
...
class SistemAnalisis:
    ...
    def _load_data(self):
        ...
        df = df.dropna(subset=["Tanggal"]) # Hapus baris dengan tanggal invalid
        # Buat kolom Bulan untuk grouping bulanan
        df["Bulan"] = df["Tanggal"].dt.to_period('M') # Format Period: Y-M
        return df
    ...
    def tren_bulanan(self, bulan=None, tahun=None):
        ...
        if tahun is None:
            tahun = df['Tanggal'].dt.year.max() if not df.empty else datetime.now().year
        df = df[df["Tanggal"].dt.year == tahun]

        if df.empty:
            return pd.Series(dtype=float)

        df_group = df.groupby(df["Tanggal"].dt.month)["Subtotal"].sum()
        # Reindex biar semua bulan ada, isi 0 kalo ga ada transaksi
        df_group = df_group.reindex(range(1, 13), fill_value=0)
    ...
    def analisis_mingguan(self):
        ...
        hari_ini = datetime.now().date()
        minggu_lalu = hari_ini - timedelta(days=6) # Ambil 7 hari terakhir
    ...
    def analisis_bulanan(self, bulan=None, tahun=None):
        ...
        if df_month.empty:
             # Jika gaada transaksi di bulan tsb, return 0
            return {
                "status": True,
                "bulan": bulan,
                "tahun": tahun,
                "total": 0,
                "jumlah_item": 0,
                "detail_kategori": {} # Tambahin detail kategori kosong
            }

        total_pendapatan = df_month["Subtotal"].sum()
        total_item_terjual = df_month["Jumlah"].sum()

        # Hitung total penjualan per kategori di bulan tsb
        detail_kategori = df_month.groupby("Kategori")["Subtotal"].sum().to_dict()
        ...
        return {
            ...
            "total": int(total_pendapatan),
            "jumlah_item": int(total_item_terjual),
            "detail_kategori": detail_kategori # Return detail kategori
        }

    # FUNGSI BARU UNTUK ANALISIS KATEGORI BULANAN
    def analisis_kategori_bulanan(self, bulan=None, tahun=None):
        """Mengembalikan data untuk pie chart kategori bulanan."""
        df = self._load_data()
        if df.empty:
            return {}
        ...
        # Group by kategori dan sum subtotal
        kategori_data = df_month.groupby("Kategori")["Subtotal"].sum().to_dict()
...
class AnalisisController:
    ...
    # FUNGSI BARU DI CONTROLLER
    def get_analisis_kategori_bulanan(self, bulan=None, tahun=None):
        return self.sistem_analisis.analisis_kategori_bulanan(bulan, tahun)
...
class GUIAnalisis(QWidget):
    ...
    def setup_layout(self):
        ...
        self.layout.addWidget(self.btn_back, 0, 2, alignment=Qt.AlignRight)
        ...
        # Tombol Baru untuk Analisis Kategori dan Generate PDF (Window Baru)
        self.btn_kategori_pdf = QPushButton("GENERATE PDF KATEGORI BULANAN")
        self.btn_kategori_pdf.setObjectName("analyzeBtn")
        self.layout.addWidget(self.btn_kategori_pdf, 1, 2)

        # Label untuk menampilkan bulan/tahun yang dipilih
        self.label_month_year = QLabel(f"Bulan/Tahun: {calendar.month_name[self.selected_month]} {self.selected_year}")
        self.label_month_year.setObjectName("monthYearLabel")
        self.layout.addWidget(self.label_month_year, 2, 0, 1, 3)

        # Widget untuk kontrol bulan/tahun (misalnya combobox)
        month_year_layout = QHBoxLayout()
        self.combo_month = QComboBox()
        self.combo_month.addItems([calendar.month_name[i] for i in range(1, 13)])
        self.combo_month.setCurrentIndex(self.selected_month - 1)
        month_year_layout.addWidget(QLabel("Bulan:"))
        month_year_layout.addWidget(self.combo_month)

        self.combo_year = QComboBox()
        current_year = datetime.now().year
        # Isi tahun dari 2020 sampe 5 tahun ke depan
        ...
        month_year_layout.addWidget(QLabel("Tahun:"))
        month_year_layout.addWidget(self.combo_year)

        self.btn_update_month_year = QPushButton("Update")
        month_year_layout.addWidget(self.btn_update_month_year)
        self.layout.addLayout(month_year_layout, 3, 0, 1, 3)

        self.fig = Figure(figsize=(8, 5))
        self.canvas = FigureCanvas(self.fig)
        self.layout.addWidget(self.canvas, 4, 0, 1, 3)
    ...
    def setup_connections(self):
        ...
        self.btn_kategori_pdf.clicked.connect(self.open_generate_pdf_dialog) # Hubungkan tombol baru ke fungsi baru
        self.btn_update_month_year.clicked.connect(self.update_month_year_selection) # Hubungkan update

    def show_analisis_mingguan(self):
        self.label_month_year.setText("Analisis Mingguan")
    ...
    def show_analisis_bulanan(self):
        self.label_month_year.setText(f"Analisis Bulanan - {calendar.month_name[self.selected_month]} {self.selected_year}")
        data = self.controller.get_tren_bulanan(self.selected_month, self.selected_year)
        if data.empty or data.sum() == 0:
        ...
        ax.set_title(f"Trend Penjualan Bulanan ({calendar.month_name[self.selected_month]} {self.selected_year})", fontweight="bold")
        ax.set_xlabel("Bulan")
        ax.set_ylabel("Total Penjualan (Rp)")
        ax.ticklabel_format(style='plain', axis='y')
        self.canvas.draw()

    def show_analisis_kategori_bulanan(self):
        """Menampilkan pie chart kategori bulanan."""
        self.label_month_year.setText(f"Analisis Kategori - {calendar.month_name[self.selected_month]} {self.selected_year}")
        kategori_data = self.controller.get_analisis_kategori_bulanan(self.selected_month, self.selected_year)

        if not kategori_data or sum(kategori_data.values()) == 0:
            QMessageBox.warning(self, "Peringatan", f"Belum ada data transaksi kategori untuk {calendar.month_name[self.selected_month]} {self.selected_year}")
            return

        self.fig.clear()
        ax = self.fig.add_subplot(111)
        labels = list(kategori_data.keys())
        sizes = list(kategori_data.values())
        # Warna-warna acak untuk pie chart
        colors_list = plt.cm.Set3(range(len(labels))).tolist()
        ax.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors_list)
        ax.set_title(f"Distribusi Penjualan per Kategori\n({calendar.month_name[self.selected_month]} {self.selected_year})", fontweight="bold", fontsize=12)
        ax.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
        self.canvas.draw()

    def open_generate_pdf_dialog(self):
        """Membuka dialog baru untuk generate PDF."""
        dialog = GUIDialogGeneratePDF(self.controller, self)
        dialog.exec_()

    def update_month_year_selection(self):
        """Memperbarui bulan dan tahun yang dipilih dan menampilkan grafik kategori."""
        self.selected_month = self.combo_month.currentIndex() + 1
        self.selected_year = int(self.combo_year.currentText())
        self.label_month_year.setText(f"Bulan/Tahun: {calendar.month_name[self.selected_month]} {self.selected_year}")
        # Secara default, tampilkan grafik kategori setelah update
        self.show_analisis_kategori_bulanan()

    def go_back(self):
        self.main_app.go_to_kasir()

# GUI DIALOG GENERATE PDF 
class GUIDialogGeneratePDF(QDialog):
    def __init__(self, controller, parent=None):
        super().__init__(parent)
        self.controller = controller
        self.setWindowTitle("Generate PDF Analisis Kategori")
        self.setModal(True) 
        self.setup_layout()
        self.setup_stylesheet()
    ...
    def setup_layout(self):
        ...
        title_label = QLabel("PILIH BULAN DAN TAHUN")
        title_label.setObjectName("dialogTitle")
        layout.addWidget(title_label)

        # Form pilihan bulan dan tahun
        form_layout = QGridLayout()
        form_layout.addWidget(QLabel("Bulan:"), 0, 0)
        self.combo_month = QComboBox()
        self.combo_month.addItems([calendar.month_name[i] for i in range(1, 13)])
        self.combo_month.setCurrentIndex(datetime.now().month - 1)
        form_layout.addWidget(self.combo_month, 0, 1)

        form_layout.addWidget(QLabel("Tahun:"), 1, 0)
        self.combo_year = QComboBox()
        current_year = datetime.now().year
        for year in range(current_year - 5, current_year + 6):
            self.combo_year.addItem(str(year))
        self.combo_year.setCurrentText(str(current_year))
        form_layout.addWidget(self.combo_year, 1, 1)

        layout.addLayout(form_layout)

        # Tombol Generate
        self.btn_generate = QPushButton("Generate PDF")
        self.btn_generate.setObjectName("generatePdfBtn")
    ...
    def setup_stylesheet(self):
        ...
        QPushButton#generatePdfBtn:hover {{
            background-color: {BUTTERCREAM};
            color: {BLUEBERRY};
        }}
    ...
    def on_generate_clicked(self):
        ...
        selected_month_index = self.combo_month.currentIndex() + 1
        selected_year = int(self.combo_year.currentText())

        self.generate_kategori_pdf(selected_month_index, selected_year)

    def generate_kategori_pdf(self, month, year):
        """Membuat file PDF dengan tabel dan pie chart kategori bulanan."""
        kategori_data = self.controller.get_analisis_kategori_bulanan(month, year)

        if not kategori_data or sum(kategori_data.values()) == 0:
            QMessageBox.warning(self, "Peringatan", f"Belum ada data transaksi kategori untuk {calendar.month_name[month]} {year}. Tidak bisa generate PDF.")
            return

        filename = f"analisis_kategori_{calendar.month_name[month].lower()}_{year}.pdf"
        ...
        title_style = ParagraphStyle(
            ...
            spaceAfter=30,
            alignment=1, 
            textColor=colors.HexColor("#2C3F70")
        )
        ...
        title_text = f"Analisis Penjualan Kategori<br/>Brewalytica Cafe<br/>{calendar.month_name[month]} {year}"
        story.append(Paragraph(title_text, title_style))
        story.append(Spacer(1, 12))

       
        table_data = [['Kategori', 'Total Penjualan (Rp)']]
        for kategori, total in kategori_data.items():
            table_data.append([kategori, f"Rp {total:,.0f}"])

        
        table = Table(table_data, colWidths=[3*inch, 2.5*inch])
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor("#C8D4E5")), 
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.HexColor("#2C3F70")), 
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 10),
            ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
            ('BACKGROUND', (0, 1), (-1, -1), colors.HexColor("#E8EBED")),
            ('GRID', (0, 0), (-1, -1), 1, colors.HexColor("#C8D4E5")),
            ('FONTSIZE', (0, 0), (-1, -1), 9),
        ]))
        story.append(table)
        ...
        sizes = list(kategori_data.values())
        colors_list = plt.cm.Set3(range(len(labels))).tolist()
        plt.pie(sizes, labels=labels, autopct='%1.1f%%', startangle=90, colors=colors_list)
        plt.title(f"Distribusi Penjualan per Kategori\n({calendar.month_name[month]} {year})", fontsize=12, fontweight='bold', pad=20)
        plt.axis('equal')
        ...
        # Embed image ke story PDF
        img = Image(temp_img_path, width=4*inch, height=4*inch)
        img.hAlign = 'CENTER'
        story.append(img)
        ...
        QMessageBox.information(self, "PDF Generated", f"Laporan analisis kategori untuk {calendar.month_name[month]} {year} telah dibuat: {filename}")
