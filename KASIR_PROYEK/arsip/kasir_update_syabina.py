...
class SistemKasir:
    ...
    def hapus_item(self, index, jumlah_hapus=1):
        ...
        if jumlah_hapus >= item["jumlah"]:
            removed_item = self.keranjang.pop(index)
            return {"status": True, "message": f"{removed_item['menu']} dihapus semua ({removed_item['jumlah']} item)"}
        
        item["jumlah"] -= jumlah_hapus
        item["subtotal"] = item["harga"] * item["jumlah"]
        return {"status": True, "message": f"{item['menu']} dikurangi {jumlah_hapus} (sisa: {item['jumlah']})"}
...
class KasirController:
    ...
    def hapus_dari_keranjang(self, index, jumlah_hapus=1):
        return self.sistem_kasir.hapus_item(index, jumlah_hapus)
...
class GUISignup(QWidget):
    ...
    def setup_layout(self):
        ...
        self.button_exit_arrow = QPushButton("âŸµ", self)
...
""")
    ...
    def hapus_item(self):
        ...
        result = self.controller.hapus_dari_keranjang(selected, jumlah_hapus=1)
